...
# ========== EVENTS MOCK ENDPOINT ==========

@app.get("/api/events/metadata")
async def get_events_metadata(limit: int = 500, start_days: int = 7):
    """Return mock event metadata for frontend testing (rows: EventID, Timestamp, Source, Level, Computer, UserSID)"""
    try:
        rows = []
        now = datetime.utcnow()
        sources = ["Security", "Application", "System", "Microsoft-Windows-Security-Auditing"]
        levels = ["Information", "Warning", "Error", "Critical"]
        event_ids = [4624, 4625, 4688, 7045, 1102, 4672, 4689]
        for i in range(max(1, min(5000, limit))):
            ts = now - timedelta(seconds=random.randint(0, max(1, start_days) * 24 * 3600))
            rows.append({
                "EventID": random.choice(event_ids),
                "Timestamp": ts.isoformat() + "Z",
                "Source": random.choice(sources),
                "Level": random.choice(levels),
                "Computer": f"HOST-{random.randint(1,50):02d}",
                "UserSID": f"S-1-5-21-{random.randint(100000,999999)}-{random.randint(1000,9999)}"
            })
        return {"rows": rows}
    except Exception as e:
        logger.error(f"Failed to construct mock event metadata: {e}")
        raise HTTPException(status_code=500, detail=str(e))
